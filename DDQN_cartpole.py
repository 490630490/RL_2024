...
def visualize(Qmodel, env, num_episodes=5):
    for _ in range(num_episodes):
        state = env.reset()
        done = False
        while not done:
            action = select_action(Qmodel, env, state, eps=0)  # 选择动作，这里 eps=0 表示完全贪心策略
            state, _, done, _ = env.step(action)  # 执行动作
            env.render()  # 渲染环境
            time.sleep(0.02)  # 添加一小段延迟以观察效果
        env.close() 

if __name__ == '__main__':
    #main()
    Q_model, performance = main()
    env = gym.make('CartPole-v1')
    visualize(Q_model, env)
